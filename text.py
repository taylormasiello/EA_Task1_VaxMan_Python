...
class Text(object):
    ...
    def setText(self, newtext): # allows to change text if needed
...
class TextGroup(object):
    def __init__(self):
        self.nextid = 10 # 0-4 used in constants
        self.alltext = {} # creates text dictionary
        self.setupText()
    ...
    def addText(self, text, color, x, y, size, time=None, id=None):
        ...
        return self.nextid # returns id used in storing text
    
    def removeText(self, id):
        self.alltext.pop(id)

    def setupText(self): # creates predefined text
        size = TILEHEIGHT
        ...
        self.alltext[GAMEOVERTEXT] = Text("GAMEOVER!", YELLOW, 10*TILEWIDTH, 20*TILEHEIGHT, size, visible=False)
        self.addText("SCORE", WHITE, 0, 0, size)
        self.addText("LEVEL", WHITE, 23*TILEWIDTH, 0, size)

    def update(self, dt): # checks for text to be updated or removed
    ...
    def updateText(self, id, value): # update text to a new value
